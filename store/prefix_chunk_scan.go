...
// PrefixChunkScanStore implements the IndexStore interface using a chunked log construction
// in Pebble with a custom prefix Comparer and Bloom filters.
type PrefixChunkScanStore struct {
	db        *pebble.DB
	chunkSize uint64
}

// NewPrefixChunkScanStore opens a Pebble DB with prefix bloom filter and split configured.
func NewPrefixChunkScanStore(dir string, chunkSize uint64) (*PrefixChunkScanStore, error) {
	if chunkSize == 0 || chunkSize > 65536 {
		return nil, fmt.Errorf("invalid chunkSize %d: must be > 0 and <= 65536", chunkSize)
	}

	comparer := *pebble.DefaultComparer
	comparer.Name = "pebble_tests.prefix_chunk_comparer"
...
func (s *PrefixChunkScanStore) Compact() error {
	return s.db.Compact([]byte{0}, []byte{0xff}, false)
}

// Close flushes all writes and closes the database.
func (s *PrefixChunkScanStore) Close() error {
	return s.db.Close()
}

// WriteBatch writes a batch of updates atomically.
func (s *PrefixChunkScanStore) WriteBatch(ctx context.Context, updates map[[32]byte][]uint64) error {
	if len(updates) == 0 {
		return nil
	}
	...
	for _, key := range keys {
		...
		if iter.SeekPrefixGE(prefix) && bytes.HasPrefix(iter.Key(), prefix) {
			for {
				...
				if !iter.Next() || !bytes.HasPrefix(iter.Key(), prefix) {
					break
				}
			}
		}
...
func (s *PrefixChunkScanStore) Lookup(ctx context.Context, key [32]byte, start uint64) ([]uint64, error) {
	prefix := make([]byte, 33)
	prefix[0] = chunkPrefix
	copy(prefix[1:], key[:])
	...
	for valid := iter.SeekPrefixGE(startKey); valid && bytes.HasPrefix(iter.Key(), prefix); valid = iter.Next() {
		k := iter.Key()
		if len(k) != 41 {
...
func (s *PrefixChunkScanStore) GetSubRoot(ctx context.Context, key [32]byte) ([32]byte, error) {
	prefix := make([]byte, 33)
	prefix[0] = chunkPrefix
	copy(prefix[1:], key[:])
	...
	if iter.SeekPrefixGE(prefix) && bytes.HasPrefix(iter.Key(), prefix) {
		...
		r, relIndices, err := deserializeChunkValue(latestVal)
		if err != nil {
			return [32]byte{}, fmt.Errorf("failed to deserialize latest chunk value for key %x: %w", key, err)
		}
		chunkNum := binary.BigEndian.Uint64(latestKey[33:])
		for _, rel := range relIndices {
			abs := chunkNum*s.chunkSize + uint64(rel)
			var idxBytes [8]byte
			binary.BigEndian.PutUint64(idxBytes[:], abs)
